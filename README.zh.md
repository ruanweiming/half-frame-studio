...
- **并发导出**：智能分块处理，避免内存溢出
- **缩略图管线**：为网格视图和拼图素材生成下采样缩略图（通过 `THUMBNAIL_MAX_WIDTH` 可配置），大幅降低内存占用
- **原图模式开关**：新增「原图模式」开关，桌面端可开启以获得最高画质，移动端默认关闭以保证稳定性
- **移动端安全并发**：根据设备类型自动限制同时处理的任务数量（如手机上显著降低并发），避免大量高像素照片导入时触发 Safari 内存崩溃
- **内存管理**：在分析/导出过程结束后主动释放 `ImageBitmap` 与 Blob URL，对长时间使用更加友好
...
- **原图模式**：电脑端可以根据需要开启「原图模式」获得更高画质；手机端建议保持关闭状态，让应用优先使用缩略图，以提升稳定性并降低崩溃风险
